...
6、	多模式：普通模式、加速模式、重力模式。重力模式为模拟黑块受到重力而下落的模式。
7、	按键音效为6个随机播放的钢琴音效。
8、	黑块的判定区域存在一定问题，移动中的黑块靠上沿的部分会被判定为白块，由于个人时间和能力有限，暂时未找到解决方法。
9、	黑块有时候会出现重叠现象，由于个人时间和精力有限，暂时未找到解决方法。
10、	点击黑块播放声音的功能在移动端无法实现，我在搜索引擎上搜索了一些资料，发现了play()方法似乎有兼容性问题，但是同样使用play()方法的网站https://bideyuanli.com/p/866却在移动端版本为55+的chrome中运行正常。由于个人时间和精力
	有限，并未研究透彻该问题，敬请见谅。
